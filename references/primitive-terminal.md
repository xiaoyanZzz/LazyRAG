...
This is a **second, fixed skin** — see [style-guide.md](style-guide.md) for the Terminal token table. It does not inherit from per-output brand tokens and isn't part of the light/dark inversion rule; every terminal example uses the same nine tokens.
...
- Brand-matched output — terminal is a fixed skin, not brand-tokenized. Do not combine it with user-supplied brand tokens unless the user explicitly chooses terminal over brand fidelity.
