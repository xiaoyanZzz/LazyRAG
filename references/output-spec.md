# LazyMind import output spec — format × size × detail × audience
...
Always generate and check the HTML first. SVG is produced from it via [`export-svg.md`](export-svg.md). Never hand-author a separate SVG file directly; the HTML is the source of truth and the artifact against which the taste gate is run. PNG is not supported in phase three because the LazyMind runtime does not bundle a browser renderer.
...
| Keynote / PowerPoint / Google Slides | `svg` | `slide-16x9` |
| Figma / Illustrator / further editing | `svg` | `fit` |
| Browser-hosted social/OG composition | `html` | `social-og` |
| Printed handout, PDF deck | `svg` | `print-a4-landscape` |
| Confluence / Notion / internal wiki | `html` | `doc-wide` |
...
| `doc-wide` | `0 0 1280 720` | 16:9 | standard | Full-width docs, wiki pages |
| `slide-16x9` | `0 0 1280 720` | 16:9 | presentation | Deck slide, projected |
| `slide-4x3` | `0 0 1024 768` | 4:3 | presentation | Legacy deck templates |
| `social-og` | `0 0 1200 632` | ~1.9:1 | presentation | Link preview card |
| `social-square` | `0 0 1080 1080` | 1:1 | presentation | Feed post, carousel |
| `print-a4-landscape` | `0 0 1120 792` | ~1.41:1 | print | A4 landscape, ~10mm margins at 96dpi |
| `print-letter-landscape` | `0 0 1056 816` | ~1.29:1 | print | US Letter landscape |
| `fit` | derived from content | any | standard | Vector hand-off; no fixed frame |
...
```svg
<text font-family="'Geist', 'PingFang SC', 'Microsoft YaHei', 'Noto Sans CJK SC', sans-serif">认证服务</text>
...
- [ ] Requested SVG produced via [`export-svg.md`](export-svg.md), not hand-authored? PNG requests clearly reported as unsupported in phase three?
