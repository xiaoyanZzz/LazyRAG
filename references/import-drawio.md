...
Load this file when the user points at a `.drawio`, `.drawio.xml`, `.drawio.png`, or `.drawio.svg` file and wants a diagram out of it — "convert this drawio", "redraw this diagram", "make this presentable", or equivalent requests.

---

## Step 1 — Extract the IR

Do not read or paste raw draw.io XML into model context. Call `find_user_attachment` and pass its local absolute `path` to the extractor:
...
For a named page use `args=["<source absolute path>", "--page", "<name or index>"]`. Use `"all"` only after the user asked for every page. Use `"--json"` only if the compact digest omitted details needed for the redraw. `args` must remain a string array, never a quoted array.
...
3. Produce an independent SVG only if requested, via [`export-svg.md`](export-svg.md), from the checked HTML. PNG is outside phase three.
...
`assets/example-import-drawio.html` is a representative redraw of a 12-node, 8-edge, 2-container source at `format=html`, `size=doc-inline`, `detail=balanced`, `audience=mixed`. It is a visual reference only; do not copy its sample facts into the user's output.
