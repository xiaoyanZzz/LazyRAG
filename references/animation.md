...
Read `assets/template-motion.html` before authoring any controlled `step` or
JavaScript-backed `reveal` diagram. Copy its `<script
data-diagram-controls>` body byte-for-byte; changing even one statement makes
the controller non-canonical and must fail validation. Use one relevant
animated example as semantic guidance:

- queue accumulation: `assets/example-queue-animated.html`
- paired policy traces: `assets/example-policy-trace-animated.html`
- secure paved road: `assets/example-paved-road-animated.html`

The example supplies motion grammar, never facts. Generated files still use
`write_file` and `save_chat_artifact`; only explicitly requested animation may
contain the canonical inline controller. No remote script, event attribute,
fetch, dynamic markup injection, or second script is allowed.
...
Use [`assets/template-motion.html`](../assets/template-motion.html) rather than inventing another controller. Its inline controller is the executable implementation contract, but the model must not reproduce that 9 KB script in `write_file`: author the semantic diagram, controls and status region without a script, then run `scripts/apply_motion_controller.py` to install the canonical controller byte-for-byte. The installer may replace one placeholder carrying only `data-diagram-controls`, but rejects modified attributes, extra scripts and malformed tags. Replace diagram content and slug-prefixed IDs, while preserving the template's state/control attributes.
...
LazyMind SVG exports are complete static-state artifacts. The exported SVG
omits HTML controls and the controller; source-visible semantic markup keeps
the result complete. PNG remains outside the current LazyMind skill boundary.

After `write_file` returns the absolute HTML path, install the controller and
then run both checks with real JSON string arrays:

```text
run_script(
  name="diagram-design",
  rel_path="scripts/apply_motion_controller.py",
  args=["<animated HTML absolute path>"]
)
run_script(
  name="diagram-design",
...
run_script(
  ...
  args=["<same absolute path>"]
)
```

LazyMind activates Skill tools per user turn. If the user sends a follow-up
asking for repair, validation or export, call `get_skill(name="diagram-design")`
again in that turn before any `run_script`; do not infer that the prior turn's
tool activation persists.

Then run `scripts/verify-geometry.py` and any applicable type-specific
validator. A `parameters error` is a tool failure, not a successful check:
stop retrying malformed variants, perform the manual checks below, and
disclose which scripts did not run.
